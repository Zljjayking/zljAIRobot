...
+ (NSDate *)fireDateWithYear:(NSInteger)year
                       month:(NSInteger)month
                         day:(NSInteger)day week:(NSInteger)week
                        hour:(NSInteger)hour
...
+ (void)localPushForDate:(NSDate *)fireDate
    ...
    if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_7_1) {
        ...
        notificationType = [application isRegisteredForRemoteNotifications];//enabledRemoteNotificationTypes
    }
...
+ (void)cancleLocalPushWithKey:(NSString *)key {
    ...
    if (notiArray) {
        for (UILocalNotification *notification in notiArray) {
            ...
            if (dic) {
                for (NSString *hkey in dic) {
                    if ([hkey isEqualToString:key]) {
