...
- (void)setTimeBroadcastView
{
    ...
    resultTimeStr=[NSString stringWithFormat:@"%@-%02ld-%02ld %@:%@:00",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)]];
  ...
  //  nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]];
    ...
    [middleSepView setBackgroundColor:TABBAR_BASE_COLOR];
...
- (void)setYearScrollView
{
    ...
    [yearScrollView setCurrentSelectPage:(yearint-1978)];
...
{
    dayScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(125, 0, 50, 190.0)];
...
{
    hourScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(170, 0, 50, 190.0)];
...
{
    minuteScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(220.5, 0, 50.0, 190.0)];
...
- (NSInteger)numberOfPages:(MXSCycleScrollView*)scrollView
{
    ...
    {
//        NSInteger index = monthScrollView.currentPage;
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    l.tag = index+1;
    if (scrollView == yearScrollView) {
        l.text = [NSString stringWithFormat:@"%ld年",1976+index];
    }
    else if (scrollView == monthScrollView)
    {
    ...
    {
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"0%ld时",index];
        }
        else
            l.text = [NSString stringWithFormat:@"%ld时",index];
    }
    else if (scrollView == minuteScrollView)
    {
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"0%ld分",index];
        }
        else
            l.text = [NSString stringWithFormat:@"%ld分",index];
    }
    else
        if (index < 10) {
            l.text = [NSString stringWithFormat:@"0%ld秒",index];
        }
        else
            l.text = [NSString stringWithFormat:@"%ld秒",index];
...
//    UILabel *secondLabel = [[(UILabel*)[[secondScrollView subviews] objectAtIndex:0] subviews] objectAtIndex:3];
    ...
    NSInteger yearInt = yearLabel.tag + 1975;
    ...
    NSString *taskDateString = [NSString stringWithFormat:@"%ld%02ld%02ld%02ld%02ld:00",yearInt,monthInt,dayInt,hourInt,minuteInt];
...
//    NSString *dateString = [NSString stringWithFormat:@"%ld%02ld%02ld%02ld%02ld:00",yearInt,monthInt,dayInt,hourInt,minuteInt];
//    NSString *weekString = [self fromDateToWeek:dateString];
   // nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d:%02d",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt,secondInt];
    ...
    resultTimeStr=[NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld:00",yearInt,monthInt,dayInt,hourInt,minuteInt];
    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *selectTimeString = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld:00",yearInt,monthInt,dayInt,hourInt,minuteInt];
