...
@implementation productChooseTableViewCell
...
- (void)initView {
    self.selectedBackgroundView.backgroundColor = kMyColor(250, 250, 250);
    ...
    [self.mechProIcon mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.DaGouBtn.mas_right).offset(0*KAdaptiveRateWidth);
    ...
    seperator.backgroundColor = VIEW_BASE_COLOR;
...
//    self.MyDelegate = [UIApplication sharedApplication].delegate;
    ...
    UIImageView *lineImg = [[UIImageView alloc] initWithFrame:CGRectMake(95*KAdaptiveRateWidth, 35, kScreenWidth-95*KAdaptiveRateWidth, 2)];
    // 调用方法 返回的iamge就是虚线
    ...
    [self.mechProTypeLB mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(point_Type.mas_right).offset(6*KAdaptiveRateWidth);
    ...
    self.dayLB.textColor = MYGRAY;
    [self.dayLB mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(point_day.mas_right).offset(6*KAdaptiveRateWidth);
    ...
    self.methodLB.textColor = MYGRAY;
    [self.methodLB mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(point_method.mas_right).offset(6*KAdaptiveRateWidth);
    ...
    [point_Goodness mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.dayLB.mas_right).offset(20*KAdaptiveRateWidth);
        make.top.equalTo(lineImg.mas_bottom).offset(11);
        make.width.mas_equalTo(3);
        make.height.mas_equalTo(3);
    }];
